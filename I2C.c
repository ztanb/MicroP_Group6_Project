...
/*
Initialization for I2C - other areas are defined in the MSP portion!
only need to call HAL_I2C_Init for everything to work in this case!

Need 16/9 duty cycle for 400kHz
*/
I2C_HandleTypeDef I2CHandle;
GPIO_InitTypeDef GPIO_Init_i2c;

//z: changes to match 'i2c_slave' projet

uint8_t RecBuffer[] = "1";
uint8_t SenBuffer[] = "1111111";
...
/*this function is automatically called by HAL_I2C_Init
This is necessary to set up the SCL and SDA pins on the i2c lines
SCL == Pin PB6, SDA == PB9
*/
void HAL_I2C_MspInit(I2C_HandleTypeDef *hi2c)
{
	...
	GPIO_Init_i2c.Pin = GPIO_PIN_6|GPIO_PIN_9; //z:changed to 6
	
	GPIO_Init_i2c.Mode = GPIO_MODE_AF_OD;
	GPIO_Init_i2c.Speed = GPIO_SPEED_FREQ_HIGH;
	GPIO_Init_i2c.Pull = GPIO_PULLUP;
	GPIO_Init_i2c.Alternate = GPIO_AF4_I2C1;
	HAL_GPIO_Init(GPIOB, &GPIO_Init_i2c);
}

/*this function obfuscates the reception of data
receive 7 bits of data: uint16_t pitch, uint16_t roll, uint_16t temperature, uint8_t doubletap
*/
...
//z: REMOVED because the other project didnt have it
/*
uint8_t* MasterReceive(void)
{
	HAL_I2C_Master_Receive(&I2CHandle,0x30E, RecBuffer, RecSize, timeout);
	return RecBuffer;
}
*/

//to change to slave, change "Master"-->"Slave", remove address

//APPLIES TO BOTH
/*this function obfuscates the transmission of data
send only the LED status, in a uint8_t --> 1 byte!
*/
/*
...
{
	HAL_I2C_Slave_Transmit(&I2CHandle, data, SenSize, timeout);
}
uint8_t* Receive(void)
{
	HAL_I2C_Slave_Receive(&I2CHandle, RecBuffer, RecSize, timeout);
	//ext_value = RecBuffer[0];
	return RecBuffer;
}
